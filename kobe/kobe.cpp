...
void assignValues(int argc, char **argv)
{
    ...
    attempt_count_per_session = static_cast<std::size_t >(stoi(arguments[1]));

    if (arguments.size() == 3)
    {
        session_count = static_cast<std::size_t >(stoi(arguments[2]));
    }
}
...
void displayResults()
{
    cout
            ...
            << " there has been "
            << success_count
            << " successes. Which means a rate of "
            << float(success_count) / session_count
            << "%"
            << endl;
}
